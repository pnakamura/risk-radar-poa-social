-- Aprimoramento da função analyze_common_causes com score composto mais inteligente
...
AS $function$
        ...
        WHEN COUNT(*) > 3 THEN
          COALESCE(
            (COUNT(*) FILTER (WHERE r.created_at >= CURRENT_DATE - INTERVAL '30 days')::numeric / 
             COUNT(*) FILTER (WHERE r.created_at >= CURRENT_DATE - INTERVAL '90 days') * 2), 1
          )
        ELSE 1
  ...
  SELECT 
    ...
    (
      ...
      (frequencia::numeric / GREATEST(1, (SELECT MAX(COUNT(*)) FROM riscos_causas GROUP BY descricao))) * 5 * 0.25 + -- Frequência normalizada 25%
  ...
  ORDER BY 
    (
      ...
      (frequencia::numeric / GREATEST(1, (SELECT MAX(COUNT(*)) FROM riscos_causas GROUP BY descricao))) * 5 * 0.25 +
